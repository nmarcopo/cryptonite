...
class TestServer(unittest.TestCase):
	...
	def test_crypto_hottest_put(self):
		...
		d["count"] = 5
		d["static"] = 'true'
		r = requests.post(self.CRYPTO_URL, data = json.dumps(d))
		self.assertTrue(self.is_json(r.content.decode()))
		resp = json.loads(r.content.decode())
		# checks return value with preloaded expected values
		self.assertEqual(resp, {"result": "success", "crypto": {"BCH": "40.97%", "XRP": "16.44%", "XLM": "11.80%", "DASH": "8.67%", "ETH": "7.71%"}, "mode": "hot"})
	...
	def test_crypto_data(self):
		...
		r = requests.put(self.CRYPTO_URL, data = json.dumps(cryptodict))
		self.assertTrue(self.is_json(r.content.decode()))
	...
	def test_crypto_if_put(self):
		self.reset_data()
		d = {}
		d["asset"] = [{"BTC":["100", "2"]}, {"DBC":["100", "100"]}]
		d["static"] = "true" 
		
		r = requests.put(self.CRYPTO_URL + "whatif/", data = json.dumps(d))
		self.assertTrue(self.is_json(r.content.decode()))
		resp = json.loads(r.content.decode())
		# checks return value with preloaded expected values
		self.assertEqual(resp, {"Net Profit": "-3327.38", "Net Percentage": "-20.24", "breakdown": [{"crypto": "BTC", "profit": "-3325.26", "profit percentage": "-20.23", "days": 100}, {"crypto": "DBC", "profit": "-2.12", "profit percentage": "-73.81", "days": 100}], "result": "success"})
